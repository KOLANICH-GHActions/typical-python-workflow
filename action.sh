...
artifactUploadCmd="env INPUT_IF-NO-FILES-FOUND=warn INPUT_RETENTION-DAYS=0 node $ARTIFACT_ACTION_DIR/dist/index.js";
...
echo "##[endgroup]";
	...
	if [ "$GITHUB_EVENT_NAME" == "push" ]; then
		...
		INPUT_NAME=$PACKAGE_FILE_NAME INPUT_PATH=$PACKAGE_FILE_PATH $artifactUploadCmd;
		echo "##[endgroup]";
	else
...
if [[ -d "$TESTS_DIR/" ]]; then
	if [ $NEED_PYTEST ]; then
		...
		$ISOLATE coverage run --branch --source=$PACKAGE_NAME -m pytest --junitxml=./rspec.xml $TESTS_DIR/*.py;
		INPUT_NAME=rspec.xml INPUT_PATH=rspec.xml $artifactUploadCmd;
	else
		echo "##[group] testing without pytest and computing coverage";
		$ISOLATE coverage run --branch --source=$PACKAGE_NAME $TESTS_DIR/*.py;
